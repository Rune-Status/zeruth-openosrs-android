...
public class Robot {
    ...
    public static void generateZoomGesture(Instrumentation inst, long startTime, boolean ifMove,
                                           Point startPoint1, Point startPoint2, Point endPoint1, Point endPoint2, int duration) {

        if (inst == null || startPoint1 == null || (ifMove && endPoint1 == null)) {
            return;
        }
        ...
        // step 2
        event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_POINTER_2_DOWN, 2,
        ...
        eventTime += EVENT_MIN_INTERVAL;
        event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_POINTER_2_UP, 2,
                properties, pointerCoords, 0, 0, 1, 1, 0, 0, 0, 0);
        inst.sendPointerSync(event);
    ...
    public synchronized void mouseMove(int x, int y) {
        ...
        MotionEvent event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_MOVE, x, y,
                metaState);
    ...
    private void mousePressByEvent() {
        ...
        final MotionEvent event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_DOWN,
                mouseX, mouseY, metaState);
    ...
    private final void dispathEventAdAPI(final MotionEvent event) {
        AndroidUIUtil.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ActivityManager.activity.dispatchTouchEvent(event);
            }
        });
    }
    ...
    private void mouseReleaseByEvent() {
        ...
        MotionEvent event = MotionEvent.obtain(downTime, eventTime, MotionEvent.ACTION_UP, mouseX,
                mouseY, metaState);
    ...
    public synchronized void keyPress(int keycode) {
        ...
        if (isInputMode) {
            ...
            if (keycode == KeyEventThreadBinder.CTRL_MASK
                    || keycode == KeyEventThreadBinder.SHIFT_MASK
                ...
                if (mask == 0) {
                    ...
                    // at :
                    // android.hardware.input.IInputManager$Stub$Proxy.injectInputEvent(IInputManager.java:417)
                    // at :
                    // android.hardware.input.InputManager.injectInputEvent(InputManager.java:685)
                    // at :
                    // android.app.Instrumentation.sendKeySync(Instrumentation.java:904)
    ...
    public synchronized BufferedImage createScreenCapture(Rectangle screenRect) {
        ...
        BufferedImage image;
        if (screenRect.x == 0 && screenRect.y == 0 && screenRect.width == b1.getWidth()
                && screenRect.height == b1.getHeight()) {
            image = new BufferedImage(b1);
        } else {
            image = new BufferedImage(Bitmap.createBitmap(b1, screenRect.x, screenRect.y,
    ...
    private final Bitmap capBitmap() {
        ...
        if (capView == null) {
            capView = UIThreadViewChanger.getContentView();// ActivityManager.applicationContext.getWindow().getDecorView();
        }
        if (capBitmap == null) {
