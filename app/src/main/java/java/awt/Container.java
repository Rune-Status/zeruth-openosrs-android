...
public class Container extends Component {
    ...
    protected synchronized void addImpl(final Component comp, final Object constraints,
                                        final int index) {
        if (comp == this) {
        ...
        {
            final ContainerEvent event = new ContainerEvent(this, ContainerEvent.COMPONENT_ADDED,
                    comp);
        ...
        if (layout != null) {
            ...
            // AbsoluteLayout.LayoutParams(0, 0, 0, 0);
    ...
    public void setLayout(final LayoutManager mgr) {
        ...
        if (layoutView == null && mgr instanceof HCCardLayout) {
            layoutView = new HCTabHost(ActivityManager.applicationContext, this);
        }
        if (layoutView == null) {
            layoutView = new AbsoluteLayout(ActivityManager.applicationContext);
        }

        // if (mgr instanceof FlowLayout) {
        // if (layoutView == null || ((layoutView instanceof RelativeLayout) ==
        // false)) {
        // layoutView = new RelativeLayout(ActivityManager.applicationContext);
        // }
        // } else if (mgr instanceof BorderLayout) {
        // if (layoutView == null || ((layoutView instanceof LinearLayout) ==
        // false)) {
        // layoutView = new LinearLayout(ActivityManager.applicationContext);
        // }
        // } else if (mgr instanceof GridLayout) {
        // if (layoutView == null || ((layoutView instanceof
        // android.widget.LinearLayout) == false)) {
        // layoutView = new
        // android.widget.LinearLayout(ActivityManager.applicationContext);
        // }
        // } else if (mgr instanceof GridBagLayout) {
        // if (layoutView == null || ((layoutView instanceof
        // android.widget.GridLayout) == false)) {
        // layoutView = new
        // android.widget.GridLayout(ActivityManager.applicationContext);
        // }
        ...
        // if (layoutView == null || ((layoutView instanceof HCTabHost) ==
        // false)) {
        // layoutView = new HCTabHost(ActivityManager.applicationContext, this);
        // }
        // } else {
    ...
    private final View buildEmptyCanvas() {
        final View p = new View(ActivityManager.applicationContext) {
            @Override
            ...
            protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
                ...
                // System.out.println("-------onMeasure setMeasuredDimension w :
                // " + measureWidth + ", h : " + measureHeight);
            ...
            private int measureHeight(final int pHeightMeasureSpec) {
                ...
                switch (heightMode) {
                    case MeasureSpec.AT_MOST:
                        // System.out.println(" height AT_MOST getSize : " +
                        // result);
                        if (Container.this.isSetMaxSize) {
                        ...
                        break;
                    case MeasureSpec.EXACTLY:
                        // System.out.println(" height EXACTLY getSize : " +
                        // result);
                        if (Container.this.isSetPrefSize) {
            ...
            protected int getSuggestedMinimumWidth() {
                if (Container.this.isSetMinSize) {
                    // System.out.println("-------getSuggestedMinimumWidth
                    // isSetMinSize : " + Container.this.minSize.width);
            ...
            protected int getSuggestedMinimumHeight() {
                if (Container.this.isSetMinSize) {
                    // System.out.println("-------getSuggestedMinimumHeight
                    // isSetMinSize : " + Container.this.minSize.height);
    ...
    public void doLayout() {
        ...
        if (isLayoutValidAdAPI()) {// container含有多个组件
            if (isUsePaintView) {
                ...
                switchParent(paintLinearLayout, layoutView);
            }
            layout();
                ...
                AndroidUIUtil.runOnUiThreadAndWait(new Runnable() {
                    ...
                    public void run() {
                        if (isUsePaintView == false) {
                            isUsePaintView = true;
                            paintLinearLayout = new LinearLayout(ActivityManager.applicationContext);
                            Container.this.addOnLayoutChangeListenerAdAPI(paintLinearLayout);
                        }

                        switchParent(layoutView, paintLinearLayout);
    ...
    public void layout() {
        ...
        synchronized (this) {
            ...
            if (layoutMgr != null) {
                L.V = L.WShop ? false
                        : LogManager.log("layoutContainer() -- " + layoutMgr.toString());
    ...
    public Dimension preferredSize() {
        if (preSize == null || !(isPreferredSizeSet() || isValid())) {
            if (layout != null) {
                ...
                // UIUtil.getViewWidthAndHeight(this.getContainerViewAdAPI(),
                // rect);//layout.preferredLayoutSize(this);
                // return rect;
    ...
    protected final boolean isLayoutValidAdAPI() {
        return layout != null && countComponents() > 0;// 因为如JPanel只含有paint(Graphics
        // g)时,countComponents()
        // == 0
    }
    ...
    /**
     * 0 represents alignment along the origin, 0.5 is centered, 1 is furthest
    ...
    public Component locate(final int x, final int y) {
        ...
        for (int i = 0; i < components.size(); i++) {
            ...
            if (comp != null) {
                ...
                // System.out.println("--------sub idx : " + i + ", Comp : " +
                // comp.getClass().getName() + ", subX:" + subX + ", subY:" +
                // subY + ", subW:" + comp.getWidth() + ", subH:" +
                // comp.getHeight());
                if (subX <= x && x <= (subX + comp.getWidth()) && subY <= y
                        && y <= (subY + comp.getHeight())) {
                    // System.out.println("------find component : " +
                    // comp.getClass().getName() + " at idx : " + i);
                    return comp;
                }
            }
        }
    ...
    Component findComponentAtImpl(final int x, final int y, final boolean ignoreEnabled) {
        ...
        for (int i = 0; i < components.size(); i++) {
            ...
            if (comp != null) {
                if (comp instanceof Container) {
                    comp = ((Container) comp).findComponentAtImpl(x - comp.getX(), y - comp.getY(),
                            ignoreEnabled);
                } else {
